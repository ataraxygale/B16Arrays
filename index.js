 ...
 * @returns {items[]} array of all items that belong to the given category
 */
function filterItemsByCategoryId(categoryId) {
  // TODO: Use the .filter() method to filter out all items that don't belong to the passed-in category
...
function logCartItems() {
  ...
  cart.forEach(cartItemId => {
    const item = findItemById(cartItemId);
